                        ...
                        <div class="col-12">
                            <div class="card mb-4">
                                <div class="card-block">
                                    <?php
                                        //Recoger datos para el alta de la empresa en la BD
                                        //No se dara de alta a una empresa que ya exista.
                                        ...
                                        while($fila = $objetoBBDD->devolverFilas())
                                        {
                                            if($razonSocial === "'".$fila["razonSocial"]."'" || $cif === "'".$fila["cif"]."'")
                                            {
                                                ...
                                                echo '<a href="altaEmpresa.php"><button type="submit" class="btn btn-primary btn-md float-right">Volver</button></a>';
                                                die();
                                            }
                                        }

                                        $sql = "INSERT INTO empresas (idEmpresa, razonSocial, direccion, cif, nombreComercial, provincia, localidad, pais, responsableEmpresa, 
                                                telefonoResponsable, telefonoEmpresa, tipoEmpresa, idTutor) VALUES (DEFAULT, ".$razonSocial.", ".$direccion.", ".$cif.", ".$nombreComercial.
                                                ", ".$provincia.", ".$localidad.", ".$pais.", ".$responsableEmpresa.", ".$telefonoResponsable.", ".$telefonoEmpresa.", ".$tipoEmpresa.", 1);";
                                        $objetoBBDD->consultarBD($sql);

                                        if($objetoBBDD->filasAfectadas()==1)
                                        {
                                            echo '<div style="color:green;font-weight: bold">Éxito al dar de alta a la empresa.</div><br>';
                                            echo '<a href="listarEmpresas.php"><button type="submit" class="btn btn-primary btn-md float-right">Volver</button></a>';
                                        }
                                        else
                                        {
                                            echo '<div style="color:red;font-weight: bold">Error al dar de alta a la empresa.</div><br>';
                                            echo '<a href="altaEmpresa.php"><button type="submit" class="btn btn-primary btn-md float-right">Volver</button></a>';
                                        }
                                    ?>
                                </div>
