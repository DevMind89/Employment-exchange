				...
				<section class="row">
					<div class="col-sm-12">
						<section class="row">
							<div class="col-12">
                                <?php
                                    //Recogida de datos a borrar y su correspondiente accion de borrado
                                    $tipo = $_POST["tipo"];
                                    $categoria = $_POST["categoria"];
                                    $subcategoria = $_POST["subcategoria"];

                                    $sql = "SELECT idFormacion FROM formacion WHERE tipo='".$tipo."' AND categoria='".$categoria."' AND subcategoria='".$subcategoria."'";
                                    $objetoBBDD->consultarBD($sql);
                                    $fila = $objetoBBDD->devolverFilas();

                                    $query = "DELETE FROM formacion WHERE idFormacion=".$fila["idFormacion"];
                                    $objetoBBDD->consultarBD($query);

                                    if($objetoBBDD->filasAfectadas()==1)
                                    {
                                        echo '<div style="color:green;font-weight: bold">Éxito al borrar la formación.</div><br>';
                                    }
                                    else
                                    {
                                        echo '<div style="color:red;font-weight: bold">Error al borrar la formación.</div><br>';
                                    }
                                    echo'<a href="listarFormacion.php"><button type="button" name="Volver" class="btn btn-primary btn-md float-right">Volver</button></a>';
                                ?>
							</div>
						</section>
					</div>
