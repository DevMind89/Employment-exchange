			...
			<main class="col-xs-12 col-sm-8 col-lg-9 col-xl-10 pt-3 pl-4 ml-auto">
				<header class="page-header row justify-center">
					<div class="col-md-6 col-lg-8" >
						<h1 class="float-left text-center text-md-left">Listado de formaciones</h1>
					</div>
							...
							<div class="col-12">
								<div class="card mb-4">
									<div class="card-block">
                                        ...
                                        <?php
                                            //Mostrar y recargar con AJAX las formaciones
                                            $sql = "SELECT * FROM formacion";
                                            $objetoBBDD->consultarBD($sql);
                                            if($fila = $objetoBBDD->devolverFilas()==0)
                                            {
                                                echo '<div style="color:red;font-weight: bold">No se han encontrado formaciones registradas.</div>';
                                            }
                                            else
                                            {
                                                ?>
                                                <form class="form" method="post" action="elegirEdicion.php">
                                                    <div class="form-group row">
                                                        <label>&nbsp&nbspTipo:&nbsp&nbsp</label>
                                                        <div>
                                                            <select class="custom-select form-control" id="tipo"
                                                                    onchange="cargarCategoria(this.value)">
                                                                <?php
                                                        ...
                                                        <label>&nbsp&nbspCategoria:&nbsp&nbsp</label>
                                                        <div>
                                                            <select class="custom-select form-control" id="categoria"
                                                                    onchange="cargarSubCategoria(this.value)"></select>
                                                        </div>

                                                        <label>&nbsp&nbspSubcategoria:&nbsp&nbsp</label>
                                                        <div>
                                                            <select class="custom-select form-control"
                                                                    id="subcategoria"></select>
                                                        </div>
                                                    </div>
                                                    <button type="submit" onclick="mod_borrar_Formacion()"
                                                            name="modificar" style="margin-left: 80%;"
                                                            class="btn btn-primary btn-md ">Modificar
                                                    </button>
                                                    <button type="submit" onclick="mod_borrar_Formacion()" name="borrar"
                                                            class="btn btn-primary btn-md float-right">Borrar
                                                    </button>
                                                </form>
                                                <?php
                                            }
