				...
				<section class="row">
					<div class="col-sm-12">
						<section class="row">
							<div class="col-12">
                                <?php
                                    //Confirmar la modificacion de la formacion seleccionada
                                    ...
                                    if($categoria == "''")
                                    {
                                        echo '<div style="color:red;font-weight: bold">Debes rellenar la categoria.</div><br>';
                                        echo '<a href="listarFormacion.php"><button type="submit" class="btn btn-primary btn-md float-right">Volver</button></a>';
                                        die();
                                    }
                                    elseif($subcategoria == "''")
                                    {
                                        echo '<div style="color:red;font-weight: bold">Debes rellenar la subcategoria.</div><br>';
                                        echo '<a href="listarFormacion.php"><button type="submit" class="btn btn-primary btn-md float-right">Volver</button></a>';
                                        die();
                                    }
                                    else
                                    {
                                        $sql = "UPDATE formacion SET tipo=".$tipo." ,categoria=".$categoria." ,subcategoria=".$subcategoria." WHERE idFormacion=".$idFormacion;
                                        $objetoBBDD->consultarBD($sql);

                                        if($objetoBBDD->filasAfectadas()==1)
                                        {
                                            echo '<div style="color:green;font-weight: bold">Éxito al modificar la formación.</div><br>';
                                        }
                                        elseif($objetoBBDD->filasAfectadas()==0)
                                        {
                                            echo '<div style="font-weight: bold">No has modificado ningún dato.</div><br>';
                                        }
                                        else
                                        {
                                            echo '<div style="color:red;font-weight: bold">Error al modificar la formación.</div><br>';
                                        }
                                        echo '<a href="listarFormacion.php"><button type="submit" class="btn btn-primary btn-md float-right">Volver</button></a>';
                                    }
